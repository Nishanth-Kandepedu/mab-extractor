...
export async function extractWithLLM(
  ...
  try {
    ...
    if (isGemini) {
      // In AI Studio, the GEMINI_API_KEY is injected into process.env at build/runtime
      const apiKey = process.env.GEMINI_API_KEY || (window as any).__GEMINI_API_KEY__;
      
      if (!apiKey || apiKey === "MY_GEMINI_API_KEY" || apiKey === "") {
        throw new Error("Gemini API key is not active. Please ensure you have clicked 'Apply changes' in the Secrets panel and then manually refresh this page.");
      }
